...
const COLUMNS: JobStatus[] = ['Saved', 'Applied', 'Screening', 'Interviewing', 'Offer', 'Rejected'];
...
export function KanbanBoard({ jobs, setJobs, onUpdateJob, onEdit, onDelete }: KanbanBoardProps) {
  ...
  const handleStatusChange = (jobId: string, newStatus: JobStatus) => {
    let updatedJobToSave: Job | null = null;
    setJobs((prevJobs) => {
      return prevJobs.map((j) => {
        if (j.id === jobId) {
          updatedJobToSave = { ...j, status: newStatus };
          return updatedJobToSave;
        }
        return j;
      });
    });
    // Let state settle, then persist DB
    setTimeout(async () => {
       if (updatedJobToSave) {
         await onUpdateJob(updatedJobToSave);
       }
    }, 0);
  };
  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={{ sideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: '0.5' } } }) }}>
        {activeJob ? (
          <JobCard job={activeJob} onEdit={onEdit} onDelete={onDelete} onStatusChange={handleStatusChange} />
        ) : null}
      </DragOverlay>
    </DndContext>
  );
